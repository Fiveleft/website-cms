...
// ---------------------------------------------------------------------------------------------------------
// Enqueue Admin Styles and JavaScript
// ---------------------------------------------------------------------------------------------------------

function fiveleft_admin_init() {
	
	/*
	// http://codex.wordpress.org/Function_Reference/wp_enqueue_script
	// http://codex.wordpress.org/Function_Reference/wp_enqueue_style
	// wp_enqueue_script( 'ajax.js', get_bloginfo('template_url') . '/assets/js/ajax.js', array( 'jquery' ) );
	wp_enqueue_script('fiveleft_jqueryui_js', get_bloginfo('template_url') . '/assets/js/libs/jquery-ui-1.8.23.custom.min.js', array( 'jquery' ) );
	wp_enqueue_script('fiveleft_plugins_js', get_bloginfo('template_url') . '/assets/js/plugins.js');
	wp_enqueue_script('fiveleft_admin_js', get_bloginfo('template_url') . '/assets/js/AscendPMAdmin.js');
	wp_enqueue_style('fiveleft_jqueryui_css', get_bloginfo('template_url') . '/assets/css/smoothness/jquery-ui-1.8.23.custom.css');
	wp_enqueue_style('fiveleft_meta_css', get_bloginfo('template_url') . '/assets/css/admin.css');
	
	// Add Custom Metaboxes on Pages:
	// http://www.farinspace.com/page-specific-wordpress-meta-box/
	$post_id = $_GET['post'] ? $_GET['post'] : $_POST['post_ID'] ;
	$template_file = get_post_meta($post_id,'_wp_page_template',TRUE);
	switch( $template_file ) 
	{
		case 'page-software-main.php' :
			require_once('custom/page-type-software-main.php');
			break;
	}
	*/
...
function update_sitedata_cache() 
{
	// include get_option('home') . '/services/getSiteData.php?uncache=1';
	// // $command = "/usr/bin/php4 -f /var/www/myweb/image_resize.php";
	// // exec( "$command > /dev/null &", $arrOutput );
	// $command = '../../../../services/getSiteData.php';
	// $output;
	// $rc;
	// exec("/bin/php " . $command . " > /dev/null 2>&1 &", $output, $rc );

	// // include $command;
	// // $escaped_command = escapeshellcmd($command);	
	// // exec( $command );
	// error_log( 'update_sitedata_cache ' . $command );
	// error_log( print_r($output, true) );
	// error_log( print_r($rc, true) );
}
add_action( 'save_post', 'update_sitedata_cache' );

// ---------------------------------------------------------------------------------------------------------
// Custom Post Types
// ---------------------------------------------------------------------------------------------------------

require_once('custom/project-post-type.php'); 
require_once('custom/client-post-type.php'); 
require_once('custom/agency-post-type.php'); 
require_once('custom/taxonomies.php'); 



// ---------------------------------------------------------------------------------------------------------
// Enqueue Front-facing Styles and JavaScript
// ---------------------------------------------------------------------------------------------------------

function fiveleft_init() {
	// Following Practice described here: http://www.garyc40.com/2010/03/5-tips-for-using-ajax-in-wordpress/#js-global
	// embed the javascript file that makes the AJAX request
	wp_enqueue_script( 'fiveleft-ajax-request', get_bloginfo('template_url') . '/js/ajax.js', array( 'jquery' ) );
	// declare the URL to the file that handles the AJAX request (wp-admin/admin-ajax.php)
	wp_localize_script( 'fiveleft-ajax-request', 'fiveleft_ajax', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ), 'nonce' => wp_create_nonce( 'fiveleft-ajax-request' ) ) );
	
	// Add Image Sizes
	// add_image_size( 'gallery-mobile', '400', '200', true );
	// add_image_size( 'gallery-tablet', '600', '300', true );
	// add_image_size( 'grid-small', '167', '150', true );
	// add_image_size( 'grid-square', '250', '250', true );
	// add_image_size( 'grid-portrait', '250', '400', true );
	// add_image_size( 'grid-landscape', '333', '250', true );

	global $wpdb;
}
add_action('init','fiveleft_init');
...
function get_posts_by_type($postType, $args) {
	$query = array_intersect( array('post_type' => $postType, 'posts_per_page' => 10 ), $args );
	return new WP_Query( array('post_type' => $postType) ); 
}  
...
function my_connection_types() {
	p2p_register_connection_type( array(
		'name' => 'project_to_client'
		,'from' => 'fiveleft_project'
		,'to' => 'fiveleft_client'
		,'admin_column' => 'from'
  		,'from_labels' => array(
    		'column_title' => 'Client'
	...
	p2p_register_connection_type( array(
		'name' => 'agency_to_client',
		'from' => 'fiveleft_agency',
		'to' => 'fiveleft_client'
	) );
}
